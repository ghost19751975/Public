...
pf_params = {'header': ['auth_key', 'email', 'password', 'accept'],
             'path': ['pet_id'],
             'query': ['filter'],
...
def pf_log(func):
    ...
    def wrapper(*args, **kwargs):
        with open('log.txt', 'a', encoding="utf8") as log_file:
            ...
            status, result, content_type = func(*args, **kwargs)
            ...
            log_file.write(f"{datetime.datetime.now()} {func.__name__}" +
                           f": status {status}, content-type '{content_type}', body {result}\n")
...
class PetFriends:
    ...
    @pf_log
    def get_api_key(self, email: str, password: str, accept: str = '*/*') -> json:
        """Метод делает запрос к API сервера и возвращает статус запроса, тип ответа и результат в формате
        ...
        try:
            ...
            result = res.text
        return status, result, ct

    @pf_log
    def get_list_of_pets(self, auth_key: json, filter: str = "", accept: str = '*/*') -> json:
        """Метод делает запрос к API сервера и возвращает статус запроса, тип ответа и результат в формате JSON
    ...
    @pf_log
    def add_new_pet(self, auth_key: json, name: str, animal_type: str, age: str, pet_photo: str, accept: str = '*/*') -> json:
        """Метод отправляет (постит) на сервер данные о добавляемом питомце и возвращает статус
        запроса на сервер, тип ответа и результат в формате JSON с данными добавленного питомца"""
    ...
    @pf_log
    def delete_pet(self, auth_key: json, pet_id: str, accept: str = '*/*') -> json:
    ...
    @pf_log
    def update_pet_info(self, auth_key: json, pet_id: str, name: str, animal_type: str, age: str, accept: str = '*/*') -> json:
    ...
    @pf_log
    def add_new_pet_without_photo(self, auth_key: json, name: str, animal_type: str, age: str, accept: str = '*/*') -> json:
    ...
    @pf_log
    def add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str, accept: str = '*/*') -> json:
